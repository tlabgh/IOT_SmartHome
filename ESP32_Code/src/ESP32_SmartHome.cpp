...
void firebaseUpdateState() {
  ...
  doc["wifi"]        = 1;
  doc["door_open"]   = doorOpen ? 1 : 0;
  doc["servo_angle"] = servoAngle;
  doc["led1"]        = led1State ? 1 : 0;
  doc["led2"]        = led2State ? 1 : 0;
  doc["led3"]        = led3State ? 1 : 0;
  doc["led4"]        = led4State ? 1 : 0;
  doc["led5"]        = led5State ? 1 : 0;

  // Nhiệt độ
  if (isnan(currentTempC)) {
    doc["temp_c"] = nullptr;      // -> JSON: null
  } else {
    doc["temp_c"] = currentTempC; // -> JSON: số float
  }
  ...
  String url = String("https://") + FIREBASE_HOST + FB_BASE_PATH + "/state.json";

  HTTPClient http;
  http.begin(url);
  http.addHeader("Content-Type", "application/json");
  int httpCode = http.PUT(json);   // PUT (ghi đè node /state)
  http.end();

  // Serial.printf("Firebase update: %d\n", httpCode);
}
...
void firebaseReadCommands() {
  ...
  String url = String("https://") + FIREBASE_HOST + FB_BASE_PATH + "/cmd.json";
  ...
  if (doc.containsKey("led1")) {
    bool st = doc["led1"] == 1;
    digitalWrite(led1, st ? HIGH : LOW);
    led1State = st;
    hadCommand = true;
  }
  if (doc.containsKey("led2")) {
    bool st = doc["led2"] == 1;
    digitalWrite(led2, st ? HIGH : LOW);
    led2State = st;
    hadCommand = true;
  }
  if (doc.containsKey("led3")) {
    bool st = doc["led3"] == 1;
    digitalWrite(led3, st ? HIGH : LOW);
    led3State = st;
    hadCommand = true;
  }
  if (doc.containsKey("led4")) {
    bool st = doc["led4"] == 1;
    digitalWrite(led4, st ? HIGH : LOW);
    led4State = st;
    hadCommand = true;
  }
  if (doc.containsKey("led5")) {
    bool st = doc["led5"] == 1;
    digitalWrite(led5, st ? HIGH : LOW);
    led5State = st;
    hadCommand = true;
  }
  ...
  if (doc.containsKey("door")) {
    int doorCmd = doc["door"];
    if (doorCmd == 1) {
      gateServo.write(SERVO_OPENED_ANGLE);
      servoAngle = SERVO_OPENED_ANGLE;
      doorOpen = true;
    } else if (doorCmd == 0) {
  ...
  if (doc.containsKey("servo_angle")) {
    int ang = constrain(int(doc["servo_angle"]), 0, 180);
    servoAngle = ang;
    gateServo.write(servoAngle);
    doorOpen = (servoAngle >= (SERVO_OPENED_ANGLE / 2));
    hadCommand = true;
  ...
  // 🔥 Sau khi xử lý, nếu có lệnh → xoá /cmd
  if (hadCommand) {
    HTTPClient httpDel;
    String delUrl = String("https://") + FIREBASE_HOST + FB_BASE_PATH + "/cmd.json";
    httpDel.setTimeout(2000);
    httpDel.begin(delUrl);
    httpDel.sendRequest("DELETE", "");   // xoá node /cmd
    httpDel.end();
  }
}
